...
const resetGeneratedSearchTables = (
  db: ReturnType<typeof getBunDatabaseClient>,
) => {
  db.exec(`
...
const populateLegacyComponents = (
  ...
  db.exec(`
    ...
    SELECT
      ...
      j.stock,
      CASE
        WHEN j.price LIKE '[%' THEN j.price
        ELSE '[' || group_concat(
          json_object(
            'qFrom', CAST(substr(price_tier.value, 1, instr(price_tier.value, '-') - 1) AS INTEGER),
            'qTo', CASE
              WHEN substr(price_tier.value, instr(price_tier.value, '-') + 1, instr(price_tier.value, ':') - instr(price_tier.value, '-') - 1) = '' THEN NULL
              ELSE CAST(substr(price_tier.value, instr(price_tier.value, '-') + 1, instr(price_tier.value, ':') - instr(price_tier.value, '-') - 1) AS INTEGER)
            END,
            'price', CAST(substr(price_tier.value, instr(price_tier.value, ':') + 1) AS REAL)
          )
        ) || ']'
      END AS price,
    ...
    LEFT JOIN json_each(
      CASE
        WHEN j.price LIKE '[%' OR j.price = '' THEN '[]'
        ELSE '["' || replace(j.price, ',', '","') || '"]'
      END
    ) AS price_tier
    JOIN categories cat
...
const createCompatibilitySearchIndex = (
  db: ReturnType<typeof getBunDatabaseClient>,
) => {
  db.exec(`
    ...
    SELECT
      ...
      REPLACE(LOWER(mfr), '', ' ')
    FROM components
    WHERE lcsc IN (1002, 7593, 11702, 15849, 74524, 965793, 965799, 2765186);
  `)
}
