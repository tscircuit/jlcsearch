...
const stripTrailingSemicolon = (statement: string) =>
...
export const createCombinedStockPropagationSql = ({
  lcscs,
  includeSearchIndex,
  derivedTableNames,
}: {
  ...
  derivedTableNames: string[]
}) => {
  const statements = [
    ...(includeSearchIndex ? [createSearchIndexPropagationSql(lcscs)] : []),
    ...derivedTableNames.map((tableName) =>
      createDerivedTablePropagationSql(tableName, lcscs),
    ),
  ]
...
if (import.meta.main) {
  const [kind, tableNameOrLcscFile, lcscFile, ...rest] = Bun.argv.slice(2)
  ...
  } else if (kind === "combined" && tableNameOrLcscFile) {
    const targetArgs = [lcscFile, ...rest].filter((value): value is string =>
      Boolean(value),
    )
    console.log(
      createCombinedStockPropagationSql({
        lcscs: readLcscBatchFile(tableNameOrLcscFile),
        includeSearchIndex: targetArgs.includes("--search-index"),
        derivedTableNames: targetArgs.filter(
          (target) => target !== "--search-index",
        ),
      }),
    )
  } else {
    throw new Error(
      "Usage: bun run scripts/stock-sync-propagation-sql.ts search_index <lcsc-file> | derived <table> <lcsc-file> | combined <lcsc-file> [--search-index] [derived-table...]",
    )
  }
}
