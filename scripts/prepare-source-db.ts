...
const objectExists = (name: string) =>
  Boolean(
    db
      .query(
...
if (objectExists("components")) {
  console.log("Source database already has components table")
  db.close()
  process.exit(0)
}

if (!objectExists("jlc_components")) {
  db.close()
  throw new Error(
    "Source database does not contain components or jlc_components tables",
  )
}

console.log("Preparing jlcparts source-db-v2 cache for jlcsearch")
