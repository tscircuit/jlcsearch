...
test("GET /led_drivers/list with voltage filter returns filtered data", async () => {
  ...
  const res = await axios.get("/led_drivers/list?json=true&supply_voltage_min=3.3&supply_voltage_max=5")
  ...
  for (const driver of res.data.led_drivers) {
    if (driver.supply_voltage_min !== null && driver.supply_voltage_max !== null) {
