...
test("GET /api/search supports is_extended_promotional filter", async () => {
  ...
  const res = await axios.get(
    "/api/search?limit=10&q=resistor&is_extended_promotional=true",
  )
...
test("GET /api/search returns is_extended_promotional field", async () => {
  ...
  const res = await axios.get("/api/search?limit=5&q=capacitor")
  ...
  if (res.data.components.length > 0) {
    ...
    expect(typeof component.is_extended_promotional).toBe("number")
  }
})
