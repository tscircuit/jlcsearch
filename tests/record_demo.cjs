...
async function main() {
  ...
  try {
    ...
    await page.goto("http://localhost:3065/components/list", {
    ...
    console.log(
      "Navigating to JSON output of components list with is_extended_promotional=true...",
    )
    await page.goto(
      "http://localhost:3065/components/list?json=true&is_extended_promotional=true&limit=10",
      {
        waitUntil: "networkidle",
      },
    )
    ...
    const videoFile = files.find(
      (f) =>
        f.endsWith(".webm") && f !== "jlcsearch_extended_promotional_demo.webm",
    )
    if (videoFile) {
      const oldPath = path.join(videoDir, videoFile)
      const newPath = path.join(
        videoDir,
        "jlcsearch_extended_promotional_demo.webm",
      )
