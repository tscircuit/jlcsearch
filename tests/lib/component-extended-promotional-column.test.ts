...
describe("componentExtendedPromotionalColumn optimization", () => {
  test("adds generated column and evaluates promotional metadata correctly", async () => {
    ...
    try {
      ...
      const testCases = [
        {
          lcsc: 1,
          extra: JSON.stringify({ library_type: "promotional" }),
          expected: 1,
        },
        {
          lcsc: 2,
          extra: JSON.stringify({ library_type: "expand_promotional" }),
          expected: 1,
        },
        {
          lcsc: 3,
          extra: JSON.stringify({ library_type: "extended_promotional" }),
          expected: 1,
        },
        {
          lcsc: 4,
          extra: JSON.stringify({ is_extended_promotional: 1 }),
          expected: 1,
        },
        {
          lcsc: 5,
        ...
        {
          lcsc: 7,
          extra: JSON.stringify({ promotional: "true" }),
          expected: 1,
        },
        {
          lcsc: 8,
          extra: JSON.stringify({ library_type: "base" }),
          expected: 0,
        },
        {
          lcsc: 9,
          extra: JSON.stringify({ library_type: "expand" }),
          expected: 0,
        },
        ...
        {
          lcsc: 12,
          extra: JSON.stringify({ title: "Regular resistor" }),
          expected: 0,
        },
      ]
  ...
  test("componentExtendedPromotionalIndex checks and executes properly", async () => {
    ...
    try {
      ...
      const beforeIndex =
        await componentExtendedPromotionalIndex.checkIfAdded(db)
