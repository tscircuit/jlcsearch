...
async function createSourceDatabaseWithLibraryType(
  libraryType: string,
  lcscId: number,
) {
...
describe("is_extended_promotional", () => {
  test("correctly parses library_type = 'promotion'", async () => {
    const { sourcePath, outputPath } =
      await createSourceDatabaseWithLibraryType("promotion", 11111)
    ...
    const catalogRow = output
      .query(
        "SELECT lcsc, basic, is_extended_promotional FROM component_catalog WHERE lcsc = 11111",
      )
      .get() as any
  ...
  test("correctly parses library_type = 'promotionextend'", async () => {
    const { sourcePath, outputPath } =
      await createSourceDatabaseWithLibraryType("promotionextend", 22222)
    ...
    const catalogRow = output
      .query(
        "SELECT lcsc, basic, is_extended_promotional FROM component_catalog WHERE lcsc = 22222",
      )
      .get() as any
  ...
  test("correctly parses library_type = 'extended_promotional'", async () => {
    const { sourcePath, outputPath } =
      await createSourceDatabaseWithLibraryType("extended_promotional", 33333)
    ...
    const catalogRow = output
      .query(
        "SELECT lcsc, basic, is_extended_promotional FROM component_catalog WHERE lcsc = 33333",
      )
      .get() as any
  ...
  test("does not set is_extended_promotional for library_type = 'base'", async () => {
    const { sourcePath, outputPath } =
      await createSourceDatabaseWithLibraryType("base", 44444)
    ...
    const catalogRow = output
      .query(
        "SELECT lcsc, basic, is_extended_promotional FROM component_catalog WHERE lcsc = 44444",
      )
      .get() as any
