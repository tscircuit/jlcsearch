...
export const componentExtendedPromotionalColumn: DbOptimizationSpec = {
  ...
  description:
    "Adds is_extended_promotional boolean column to components table, true when preferred and not basic",
  ...
  async execute(db: KyselyDatabaseInstance) {
    await sql`
      ...
      GENERATED ALWAYS AS (CASE WHEN preferred = 1 AND basic = 0 THEN 1 ELSE 0 END)
    `.execute(db)
