...
export const ledMatrixTableSpec: DerivedTableSpec<LEDMatrix> = {
  tableName: "led_matrix",
  extraColumns: [
    { name: "package", type: "text" },
    { name: "matrix_size", type: "text" },
    { name: "color", type: "text" },
    { name: "led_type", type: "text" },
  ],
  ...
  mapToTable(components) {
    return components.map((c) => {
      try {
        ...
        // Extract LED type from description or attributes
        let led_type = attrs.Type || undefined
        if (!led_type) {
          if (c.description.includes("SMD")) led_type = "SMD"
          else if (c.description.includes("THT")) led_type = "Through Hole"
        }
