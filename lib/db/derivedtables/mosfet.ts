    ...
    return components.map((c) => {
      try {
        ...
        return {
          ...
          drain_source_voltage: parseValue(attrs["Drain Source Voltage (Vdss)"]),
          continuous_drain_current: parseValue(attrs["Continuous Drain Current (Id)"]),
          gate_threshold_voltage: parseValue(attrs["Gate Threshold Voltage (Vgs(th)@Id)"]),
