...
export const imuTableSpec: DerivedTableSpec<Imu> = {
  ...
  listCandidateComponents: (db) =>
    db
      ...
      .where((eb) =>
        eb.or([
          eb("categories.subcategory", "=", "IMUs"),
          eb("categories.subcategory", "=", "Inertial Measurement Units"),
          eb("categories.subcategory", "like", "%IMU%"),
        ]),
      ),
  mapToTable: (components) => {
    return components.map((c): Imu | null => {
      ...
      const sensorText = `${desc} ${c.mfr.toLowerCase()}`
      const hasAccelerometer =
        sensorText.includes("accel") || sensorText.includes("gyro+accel")
      const hasGyroscope = sensorText.includes("gyro")
      const hasMagnetometer =
        sensorText.includes("magnet") || sensorText.includes("compass")
