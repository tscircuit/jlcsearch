...
SELECT
  ...
  basic,
  preferred,
  CASE WHEN basic = 0 AND preferred = 1 THEN 1 ELSE 0 END AS is_extended_promotional,
  description,
  stock,
  price,
  extra
...
CREATE INDEX IF NOT EXISTS idx_component_catalog_is_extended_promotional_stock ON component_catalog(is_extended_promotional, stock DESC);
